...
// pull in Mongoose model for businesses
...
// { business: { title: '', text: 'foo' } } -> { business: { text: 'foo' } }
...
router.get('/businesses', requireToken, (req, res, next) => {
  Business.find({owner: req.user.id})
    .then(businesses => {
      // `businesses` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return businesses.map(business => business.toObject())
    })
    // respond with status 200 and JSON of the businesses
...
router.delete('/businesses/:id', requireToken, (req, res, next) => {
  Business.findById(req.params.id)
    .then(handle404)
    .then(business => {
      // throw an error if current user doesn't own `business`
      requireOwnership(req, business)
      // delete the business ONLY IF the above didn't throw
      business.deleteOne()
    })
